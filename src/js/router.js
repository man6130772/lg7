...
+function ($) {
  ...
  var Router = function() {
    ...
    /**
     *  Date: 2017.01.11 11:54
     *  Author: chen.cheng
     *  Description: stateid存在则不需要加1
     */
    // this.state.setItem("stateid", parseInt(this.state.getItem("stateid") || 1)+1);
    this.state.setItem("stateid", parseInt(this.state.getItem("stateid") || 1));
    this.state.setItem("currentStateID", parseInt(this.state.getItem("currentStateID") || this.state.getItem("stateid")));
    this.state.setItem("currentHref", location.href)
    this.stack = sessionStorage;
    /**
     *  Date: 2017.01.11 11:17
     *  Author: chen.cheng
     *  Description: 路由修改，如果forward，back存在history则不删除，不存在则重置
     */
    if(!this.state.getItem("back")) this.stack.setItem("back", "[]");
    if(!this.state.getItem("forward")) this.stack.setItem("forward", "[]");
  ...
  Router.prototype.init = function() {
    ...
    if(!state) {
      ...
      var id = this.state.getItem("stateid");
     ...
     *  Date: 2017.01.10
     *  Author: chen.cheng
     *  Description: load事件超时无法加载下面的方法，改为jquery的ready事件
     **/
      ...
      for(var i=0;i<forward.length;i++) {
        $(forward[i].pageid).each(function() {
          ...
          if($page.data("page-remote")) {
             ...
             *  Date: 2016.11.12
             *  Author: chen.cheng
             *  Description: 解决路由事件中，若id相同，forward记录强制删除了所有page，导致新page无法插入的bug
             **/
            if($page[0].id == newPageid) return true;
            $page.remove();
          }
        });
      }
  ...
  Router.prototype._back = function() {
     ...
     *  Date: 2016.12.13
     *  Author: chen.cheng
     *  Description: 如果找不到节点重新loadPage
     **/
    if(!newPage[0]) {
      this.getPage(h.url, function(page, extra) {
        var pageid = currentPage[0].id;
        var newPageid = page[0].id;

        var self = this;

        this.pushForward({url: this.getCurrentHref(), pageid: "#"+currentPage[0].id, id: this.getCurrentStateID(), animation: h.animation});

        page.insertBefore(currentPage[0]);
        ...
        this.animatePages(page, this.getCurrentPage(), true, !h.animation);
  ...
  Router.prototype._forward = function() {
    ...
    if(!newPage[0]) {
      this.getPage(h.url, function(page, extra) {
        ...
        this.pushBack({url: this.getCurrentHref(), pageid: "#"+currentPage[0].id, id: this.getCurrentStateID(), animation: h.animation});
        ...
        this.animatePages(this.getCurrentPage(), page, false, !h.animation);
  ...
  Router.prototype.getCurrentHref = function() {
    return this.state.getItem("currentHref");
  }
  Router.prototype.setCurrentHref = function() {
    this.state.setItem("currentHref", location.href);
  }
